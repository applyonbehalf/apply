...
class DatabaseConnection:
    ...
    async def update_application_status(self, app_id: str, status: str, error_message: str = None) -> bool:
        ...
        try:
            self.supabase.rpc('update_application_status', {
                'app_id': app_id,
                'new_status': status,
    ...
    async def get_user_applications_since(self, user_id: str, since_date) -> List[dict]:
    ...
    async def update_batch_progress(self, batch_id: str, processed_count: int, successful_count: int, failed_count: int) -> bool:
        ...
        try:
            ...
            batch = self.supabase.table('application_batches').select('total_count').eq('id', batch_id).execute()
            if batch.data and processed_count >= batch.data[0]['total_count']:
